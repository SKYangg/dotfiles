...
- Before preserving, replacing, or adding a mechanism, test the simpler
  baselines: delete it, use a native capability, compose existing capabilities
  directly, or make no change. Reject these only for an observed requirement or
...
- Inspect the working tree before broad edits. Treat pre-existing changes as
  source-unattributed: preserve them, but do not infer user intent, requirements,
  authorship, or permission from their presence.
- Never discard, overwrite, revert, or clean up unrelated work. Do not use
